...
- Track limit overlays are loaded from `assets/trackmaps/<trackId>.json`, where `trackId` is the normalized track name (lowercase, non-alphanumerics replaced with `_`).
- Each JSON file follows the “simple” schema generated by `admin/simple-track-map.html`: a `meta` block plus `left`, `right`, and `center` arrays of `{ "x": number, "y": number }` points (thousands of entries per edge).
- The loader converts those objects into tuples internally, so as long as the arrays exist the viewer can render the limits; no legacy `leftEdge/rightEdge` fields are required anymore.
- To preview or regenerate a map, drop calibration laps into `admin/simple-track-map.html`, then save the exported JSON under the expected track ID.
